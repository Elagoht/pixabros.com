...
// returning (media_id, label) for every row that fills it in.
type source struct {
	module string
	query  string
}
...
var sources = []source{
	{"games", `SELECT cartridge_art_id, title FROM games WHERE cartridge_art_id IS NOT NULL`},
	{"games", `SELECT cd_cover_art_id, title FROM games WHERE cd_cover_art_id IS NOT NULL`},
	{"games", `SELECT og_image_id, title FROM games WHERE og_image_id IS NOT NULL`},
	{"games", `SELECT s.media_id, g.title FROM game_screenshots s JOIN games g ON g.id = s.game_id`},
	{"members", `SELECT avatar_id, name FROM members WHERE avatar_id IS NOT NULL`},
	{"devlog", `SELECT og_image_id, title FROM devlog_posts WHERE og_image_id IS NOT NULL`},
	{"awards", `SELECT picture_id, title FROM awards WHERE picture_id IS NOT NULL`},
	...
	// and have to be listed explicitly.
	{"site-settings", `SELECT value, key FROM site_settings WHERE value_type = 'media' AND value != ''`},
	{"homepage", `SELECT value, key FROM homepage_settings WHERE value_type = 'media' AND value != ''`},
}
...
func Lookup(db *sql.DB) (map[string][]Usage, error) {
	...
	for _, src := range sources {
		...
		for rows.Next() {
			...
			usages[mediaID] = append(usages[mediaID], Usage{Module: src.module, Label: label})
		}
